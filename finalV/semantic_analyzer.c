    ...
    switch (expr_node->type) {
        ...
        case NODE_FUNCTION_CALL: {
            ...
            if (func_name_node->type == NODE_IDENTIFIER) { // Standard function call by name
                ...
                // Argument checking for specific built-ins
                if (strcmp(func_name_str, "print_number") == 0) {
                    if (!expr_node->data.function_call.arg_list || expr_node->data.function_call.arg_list->next != NULL) {
                        semantic_error("print_number expects 1 argument.", expr_node->line_number); return TYPE_KIND_ERROR;
                    }
                    TypeKind arg_type = analyze_expression(expr_node->data.function_call.arg_list->node);
                    if (arg_type != TYPE_KIND_NUMBER && arg_type != TYPE_KIND_ERROR) { // Allow error to propagate
                        semantic_error("Argument to print_number must be a number.", expr_node->line_number); return TYPE_KIND_ERROR;
                    }
                    // No specific taint propagation for print_number's result (it's void)
                    return TYPE_KIND_VOID; // print_number returns void
                } else if (strcmp(func_name_str, "print_text") == 0) {
                     if (!expr_node->data.function_call.arg_list || expr_node->data.function_call.arg_list->next != NULL) {
                        semantic_error("print_text expects 1 argument.", expr_node->line_number); return TYPE_KIND_ERROR;
                    }
                    TypeKind arg_type = analyze_expression(expr_node->data.function_call.arg_list->node);
                    if (arg_type != TYPE_KIND_TEXT && arg_type != TYPE_KIND_ERROR) {
                        semantic_error("Argument to print_text must be text.", expr_node->line_number); return TYPE_KIND_ERROR;
                    }
...
static ValueRange get_expression_range_and_taint(ASTNode* expr_node, int* out_is_tainted) {
    ...
    switch(expr_node->type) {
        ...
        case NODE_IDENTIFIER: {
            SymbolTableEntry* entry = symbol_table_lookup(expr_node->data.identifier_name);
            if (entry->kind == SYMBOL_VARIABLE && !entry->is_initialized) {
                // This check is for the variable itself.
                // For arrays, this means the array variable might not have had any elements assigned yet.
                char msg[100]; sprintf(msg, "Variable '%s' may be used before initialization.", expr_node->data.identifier_name);
                semantic_warning(msg, expr_node->line_number);
            }
            break;
        ...
        case NODE_FUNCTION_CALL: {
            vr = range_unknown(); // Range of function call result is usually unknown statically
            ...
            // Specific taint handling for built-ins if needed
            if (expr_node->data.function_call.function_name_or_expr->type == NODE_IDENTIFIER) {
                const char* func_name = expr_node->data.function_call.function_name_or_expr->data.identifier_name;
                if (strcmp(func_name, "read_number") == 0) { // This shouldn't be hit if NODE_READ_NUMBER_CALL is used
                    if(out_is_tainted) *out_is_tainted = 1;
                    vr.is_tainted = 1;
                }
                // else if (strcmp(func_name, "some_sanitizer") == 0) {
                //    if(out_is_tainted) *out_is_tainted = 0; // Sanitizer removes taint
                //    vr.is_tainted = 0;
                // }
            }
            // If vr.is_known is set (e.g. for a const-returning function), then update its taint
            if (vr.is_known && out_is_tainted) vr.is_tainted = *out_is_tainted;
            break;
        }
